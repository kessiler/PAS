...
<div id="main">
    ...
    <div id="site_content">
                    ...
                    <form id="relacDieta" method="post" action="" style="margin: 10px 0 0 0">
                        <p><span>Dieta(s)</span>
                            <select id="sele" name="sele" style="height: 22px;">
                                ...
                                <option value="<?=$value['CDDIETA'];?>"><?=$value['NMDIETA'];?></option>
                                <? endforeach;?>
                            </select>
                        </p>
                        <div id="table">
        ...
        if(isset($_POST['submit'])) {
            $Rel = new Relatorios();
            $Rel->setNameRel("relacaodieta".date('Y-m-d-G-i-s').".pdf");
            if(isset($_POST['submitA'])) {
                if(isset($_POST['relaccheck'])) {
                    $Rel->RelacaoDietas(array('dieta' => $_POST['sele'], 'relacoes' => $_POST['relaccheck']), 'I');
                }  else {
                ...
                if(isset($_POST['submitB'])) {
                    if(isset($_POST['relaccheck'])) {
                        $Rel->RelacaoDietas(array('dieta' => $_POST['sele'], 'relacoes' => $_POST['relaccheck']), 'D');
                    } else {
                        $Error->setError('Não há dados para ser gerado o relatório!');
                        $Error->ShowError();
                    }
